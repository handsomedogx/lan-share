...
│   ├── file-search-test.js  仓库「按文件名搜索」的纯逻辑测试（vm + 假 DOM，无接口可断言）
│   ├── make-preview.js      生成静态 UI 预览（无截图环境的交付路径，两份状态）
...
> 未登录时前端只把**上传按钮**置灰，另加一条 `.lock-banner` 提示条说明原因；
> 提示条必须**留在文档流里**（在列表上方占一行），列表和下载按钮照常可见可点。
> 别做成绝对定位的浮卡 —— 试过：卡片从面板右上角往下悬垂约 200px，
> 正好把表格最右侧的「上传者 / 下载」两列压住，未登录用户在最需要下载时点不到下载。
> 也不要把整块面板糊住 —— 那会连「看和拿」一起挡住。
...
**文件置顶（置顶项在列表最前）** · **按文件名搜索（纯前端过滤，空格分隔多关键词取交集，
命中片段高亮）** · SQLite · nginx 反代 · procd 启动

**刻意未做**（按文档要求）：WebRTC、分片/秒传/断点续传、多级文件夹、
复杂权限（当前只有 admin/user 两级）、聊天记录持久化、Office 预览、视频预览、PWA。

文件名搜索**不做服务端接口**：列表本来就是一次性全量拿到的（`/api/files`），
前端过滤一次的成本远低于一次往返，路由器上少一条按关键词扫库的查询路径。
所以搜索词也不进 URL、不落任何状态，刷新即回到全量。
...
```sh
node scripts/file-search-test.js
```

搜索完全在前端做，**没有任何接口可以断言**，端到端脚本够不着它。所以这个脚本用
`vm` 把**真实的** `web/js/app.js` 跑在假 `document` 上，只取三个纯函数
（`searchTokens` / `nameMatches` / `highlightedName`）来测 —— 不复制实现，
复制出来的那份不会跟着源文件一起改。当前 **27 条断言**，全绿。
