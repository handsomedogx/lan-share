...
│   ├── cleanup/             过期文件 / 无主聊天文件 / 会话清理
│   └── api/                 路由与全部 handler（含 chat_files.go）
│
├── web/                     前端源文件（同步到 cmd/server/web/）
│
├── deploy/
│   ├── lan-share.init       /etc/init.d/lan-share（procd，nginx 反代模式）
│   ├── lan-share-direct.init /etc/init.d/lan-share（procd，直连 0.0.0.0 模式）
│   └── nginx-lan-share.conf nginx 反代配置
│
├── scripts/
│   ├── build.bat            Windows 一键交叉编译
...
- **`ttlMinutes`（存活时长）**：只接受白名单档位 `10 / 60 / 360 / 1440 / 0`，
  非白名单值 → `400`。留空默认 `60`（1 小时）。**`0` 表示不限时**。
- 响应带 `expiresAt`（毫秒时间戳，`0` 表示不限时）与 `remainingSeconds`
  （`-1` 表示不限时，`0` 表示已过期）。
...
| `temporary` | 兼容旧版遗留 | 按 `expiresAt` 到期，**当前 UI 不再写入** |
...
> 仓库**只有永久文件**，没有「临时/永久」分类切换。上传一律 `kind=permanent`。
> 后端仍保留 `temporary` 的读写能力（供 API 直接调用与向后兼容），但 UI 不再暴露。
...
```json
{ "event": "hello",   "code": "K7MF", "self": "127.0.0.1", "online": 2, "members": ["a","b"], "history": [], "expiresAt": 0, "remainingSeconds": -1 }
...
├── files/
│   ├── permanent/             文件仓库（长期保存）
│   ├── temporary/             兼容旧版遗留的临时目录，当前 UI 不再写入
│   └── chat/                  聊天室文件，随房间销毁而删除
└── logs/
...
房间号大小写不敏感、不限时房间的 `-1` / `0` 语义。当前 **15 条断言**，全绿。
...
- `TestUnlimitedRoomSurvivesReap`：`ttl=0` 的房间不因「过期」被删，但空闲超时会被回收
